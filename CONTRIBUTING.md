...
| `version` | semver of the mod itself; bump it when the patches change |
| `upstream.ref` / `upstream.commit` | the release tag and exact commit the patches apply to |
...
Bump `version` in `mod.json` and open a PR. The nightly compatibility job reports which mods no longer apply to the latest release, so you will usually learn about breakage before users do.
